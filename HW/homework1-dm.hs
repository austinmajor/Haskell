...
leap n = (n `mod` 4 == 0) && (not (n `mod` 100 == 0) || (n `mod` 400 == 0))
...
parse id = [[idCopy | idCopy <- id, idCopy `elem` (['A'..'Z'] ++ ['a'..'z'])] ++ [idCopy | idCopy <- id, not (idCopy `elem` ['A'..'Z'])]]
